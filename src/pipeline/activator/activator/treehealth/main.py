...
import requests
import torch
from activator.utils.progress import (DownloadProgressBar, ProgressPercentage,
                                      timing, warp_callback)
...
def get_aviris_cog_collection(aviris_collection_id):
    collection_id, description = collection_id_to_collection_id(
        aviris_collection_id)
    collection = pystac.Collection(collection_id,
                                   description,
                                   pystac.Extent(
...
def cli_parser():
    ...
    parser.add_argument("--aviris-collection-id", type=str, default=AVIRIS_ARCHIVE_COLLECTION_ID)
    parser.add_argument("--aviris-stac-id", type=str, default="f140603t01p00r18_sc01", help="STAC Item ID to process from the STAC collection")
    ...
    parser.add_argument("--s3-bucket", type=str, default=os.environ.get("S3_BUCKET", "aviris-data"))
    parser.add_argument("--s3-prefix", type=str, default=os.environ.get("S3_PREFIX", 'tree-health'))
    parser.add_argument("--stac-api-uri", type=str, default=os.environ.get("STAC_API_URI", "http://franklin:9090"))
    parser.add_argument("--temp-dir", type=str, default=os.environ.get("TEMP_DIR", '/tmp'))
    ...
    parser.add_argument("--keep-temp-dir", action="store_true", help="If provided, script does not delete temporary directory before script exits. Useful for debugging.")
    parser.add_argument("--skip-compute", action="store_true", help="Skip downloading and inference (useful for debugging).")
    return parser
...
def main():
    ...
    item = stac_client.get_collection_item(args.aviris_collection_id, args.aviris_stac_id)
